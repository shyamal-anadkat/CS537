...
#
include < stdlib.h > #include < stdio.h > #include < string.h > #include < errno.h > #include < sys / wait.h > #include < unistd.h > #include < ctype.h > #include < sys / types.h > #include < sys / stat.h > #include < fcntl.h >
...
int parseCmd(char * in , Cmd * cmd) {
  ...
  //trim trailing space 
  ret = in +strlen( in ) - 1;
  while (ret > in && isspace((unsigned char) * ret)) ret--; * (ret + 1) = 0;
  ...
  //can't start with < , >, & 
  if (str_equal(res[0], "<") || str_equal(res[0], ">") || str_equal(res[0], "&") || str_equal(res[numArgs - 1], ">") || str_equal(res[numArgs - 1], "<")) {
...
void redirHandler(Cmd cmd) {
  ...
  pid_t pid;
  char * * parsd = NULL;
  parsd = malloc(sizeof(char * ) * sizeof(cmd.arg));
...
int main(int argc, char * * argv) {
  ...
  INTERACTIVE_START: while (1) {
    ...
    if (fgets(buffer, MAX_ARGS, stream) != NULL) {
      ...
      if (isbg) {
        int bg_status;
        waitpid(getpid(), & bg_status, WNOHANG);
        fflush(stdout);
        fprintf(stderr, "[%s (%d) completed with status %d]\n", bg.name, getpid(), bg_status);
        fflush(stdout);
        isbg = 0;
      }
